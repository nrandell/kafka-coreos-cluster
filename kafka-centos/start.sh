...
sed -e "s/^broker.id=.*$/broker.id=$BROKER_ID/" -e "s/^zookeeper.connect=.*$/zookeeper.connect=$ZOOKEEPERS/" $KAFKA_HOME/config/server.properties.template > $KAFKA_HOME/config/server.properties
