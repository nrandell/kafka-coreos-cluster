...
if [ "x${BROKER_HOST}" = "x" ]
then
	echo "BROKER_HOST must be set"
	exit 1
fi


sed \
	-e "s/^broker.id=.*$/broker.id=$BROKER_ID/" \
	-e "s/^zookeeper.connect=.*$/zookeeper.connect=$ZOOKEEPERS/" \
	-e "s/^advertised.host.name=.*$/advertised.host.name=$BROKER_HOST/" \
